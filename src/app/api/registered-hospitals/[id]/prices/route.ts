...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const res = await fetch(`${B}/api/registered-hospitals/${id}/prices`, { headers: fwd(_req) });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const res = await fetch(`${B}/api/registered-hospitals/${id}/prices`, {
            method: "POST",
        ...
        return Response.json({ error: "Failed to add price" }, { status: 500 });
    }
}
